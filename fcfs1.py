...
for i in range(1,int(n)+1):
    at = input(f'Enter Arrival time of P{i} ')
    bt = input(f'Enter Burst Time of P{i} ')
    processes.append(Process(f'P{i}', int(at),int(bt)))
...
class FCFS:
    ...
    def display(self):
        print(f'\nPID\tAT\tBT\tCT\tTAT\tWT')
        self.processes.sort(key = lambda x:x.pid[1:])
        avg_tat = 0
        avg_wt = 0
        for p in self.processes:
            print(f'{p.pid}\t{p.at}\t{p.bt}\t{p.ct}\t{p.tat}\t{p.wt}')
